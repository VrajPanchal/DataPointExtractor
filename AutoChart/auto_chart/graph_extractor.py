        ...
        if not is_anomaly:
            ...
            for i, group in enumerate(groups):
                ...
                if not is_start_point:
                    while i >= len(running_avg_heights):
                        running_avg_heights.append(height)  # or use a default value
...
plt.imshow(contour_img_updated)
plt.show()
...
def detect_axis_scale_type(tick_values):
    """
    Detect if an axis is logarithmic or linear based on tick values.
    Returns 'log' or 'linear'.
    """
    # If any value is in exponential notation, treat as log
    for v in tick_values:
        if isinstance(v, str) and (re.search(r'e[\+\-]?\d+', v) or '^' in v):
            return 'log'
    # Try to convert all to float
    try:
        values = [float(str(v).replace('^', 'e')) for v in tick_values]
    except Exception:
        return 'linear'
    # If all values are > 0 and log10 differences are more uniform than linear differences, treat as log
    if all(val > 0 for val in values):
        diffs = [values[i+1] - values[i] for i in range(len(values)-1)]
        log_diffs = [np.log10(values[i+1]) - np.log10(values[i]) for i in range(len(values)-1)]
        if np.std(log_diffs) < np.std(diffs):
            return 'log'
    return 'linear'


def pixel_to_value(pixel, min_pixel, max_pixel, min_value, max_value, scale_type):
    if scale_type == 'log':
        minv, maxv = np.log10(min_value), np.log10(max_value)
        value = 10 ** (minv + (pixel - min_pixel) / (max_pixel - min_pixel) * (maxv - minv))
        return value
    else:
        return min_value + (pixel - min_pixel) / (max_pixel - min_pixel) * (max_value - min_value)


def value_to_pixel(value, min_pixel, max_pixel, min_value, max_value, scale_type):
    if scale_type == 'log':
        minv, maxv = np.log10(min_value), np.log10(max_value)
        val_log = np.log10(value)
        pixel = min_pixel + (val_log - minv) / (maxv - minv) * (max_pixel - min_pixel)
        return int(round(pixel))
    else:
        pixel = min_pixel + (value - min_value) / (max_value - min_value) * (max_pixel - min_pixel)
        return int(round(pixel))


def find_axis_scales_and_types(min_x_tickmark, max_x_tickmark, min_y_tickmark, max_y_tickmark, measures):
    # Extract tick values using OCR
    text_measures = []
    for i, measure in enumerate(measures):
        text, _, _ = get_text_in_region(original_image, measure, i)
    ...
    # Parse values
    try:
        max_y_measure, min_y_measure = float(str(text_measures[0]).replace('^', 'e')), float(str(text_measures[1]).replace('^', 'e'))
        min_x_measure, max_x_measure = float(str(text_measures[2]).replace('^', 'e')), float(str(text_measures[3]).replace('^', 'e'))
    except Exception:
        max_y_measure, min_y_measure, min_x_measure, max_x_measure = 1, 0, 0, 1
    # Detect axis scale types
    x_scale_type = detect_axis_scale_type([text_measures[2], text_measures[3]])
    y_scale_type = detect_axis_scale_type([text_measures[1], text_measures[0]])
    # Get pixel positions
    min_x_pixel, max_x_pixel = min_x_tickmark[0], max_x_tickmark[0]
    min_y_pixel, max_y_pixel = min_y_tickmark[1], max_y_tickmark[1]
    origin = (min_x_pixel, max_y_pixel)
    return {
        'x': {
            'min_pixel': min_x_pixel,
            'max_pixel': max_x_pixel,
            'min_value': min_x_measure,
            'max_value': max_x_measure,
            'scale_type': x_scale_type
        },
        'y': {
            'min_pixel': max_y_pixel,  # y axis is usually top to bottom
            'max_pixel': min_y_pixel,
            'min_value': min_y_measure,
            'max_value': max_y_measure,
...
def scale_points_logaware(start_points, bitmap, intersection_starts, intersection_ends, axis_info):
    final_points = {}
    for j, start_point in enumerate(start_points):
        scaled_points = []
        traced_curve = trace_curve(start_point, bitmap, intersection_starts, intersection_ends)
        for point in traced_curve:
            ...
            # X axis
            x_val = pixel_to_value(px, axis_info['x']['min_pixel'], axis_info['x']['max_pixel'],
                                   axis_info['x']['min_value'], axis_info['x']['max_value'], axis_info['x']['scale_type'])
            # Y axis (note: y axis is usually inverted in images)
            y_val = pixel_to_value(py, axis_info['y']['min_pixel'], axis_info['y']['max_pixel'],
                                   axis_info['y']['min_value'], axis_info['y']['max_value'], axis_info['y']['scale_type'])
            if x_val >= 0 and y_val >= 0:
                scaled_points.append((x_val, y_val))
        final_points[j] = scaled_points
    return final_points

# ... existing code ...
# Replace old scale finding and scaling with new log-aware versions
axis_info = find_axis_scales_and_types(min_x_tickmark, max_x_tickmark, min_y_tickmark, max_y_tickmark, measures)
final_points = scale_points_logaware(start_points, bitmap, intersection_starts, intersection_ends, axis_info)
...
print(f'X-scale: {axis_info["x"]["scale_type"]} per pixel')
print(f'Y-scale: {axis_info["y"]["scale_type"]} per pixel')
print(f'Origin: {axis_info["origin"]}')
