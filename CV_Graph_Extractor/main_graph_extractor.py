...
if __name__ == "__main__":
    ...
    # Remove grid lines (and other pixels in 80-255 range) BEFORE grayscale/binary conversion
    print("Removing pixels in the 80-255 range...")
    processed_image = remove_pixels(original_image.copy())
    print("Pixels removed.")
    # Now convert to black and white (binary) after grid lines are removed
    _, processed_bw = cv2.threshold(processed_image, 127, 255, cv2.THRESH_BINARY)
    plt.imshow(processed_bw, cmap='gray')
    plt.show()
    print("Detecting axes...")
    x_axis, y_axis = detect_axes(processed_bw)
    ...
    result_image = cv2.cvtColor(processed_bw, cv2.COLOR_GRAY2BGR)
    ...
    min_x_tickmark, max_x_tickmark, min_y_tickmark, max_y_tickmark = find_tickmarks(processed_bw, x_axis, y_axis)
    ...
    cleaned_image = remove_axes_and_ticks(processed_bw, x_axis, y_axis)
    ...
    original_with_traces = trace_original_image(cleaned_image, result_image, x_axis, y_axis, curves, 
                                                           min_x_tickmark, max_x_tickmark, min_y_tickmark, max_y_tickmark,
                                                           measures, processed_bw,original_image)
